 ...
 * <pre>
 * FixedPointDataSource<Temperature> dataSource = new FixedPointDataSource<>(header, "temp", 0, Temperature.class, fileChannel, dataAddress);
 * Temperature[] temps = dataSource.readAll();
 * dataSource.stream().forEach(t -> System.out.println(t.temperature));
 * </pre>
 */
public class FixedPointDataSource<T> extends AbstractFixedPointDataSource<T> {
    ...
    /**
     * Constructs a FixedPointDataSource for buffer operations and streaming from an HDF5 file.
     *
     * @param headerPrefixV1 the HDF5 object header prefix
     * @param name the name of the field in T to map to the dataset
     * @param scale the scale for BigDecimal values; 0 for BigInteger
