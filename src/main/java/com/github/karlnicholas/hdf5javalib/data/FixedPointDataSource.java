 ...
 * <pre>
 * FixedPointDataSource rawSource = new FixedPointDataSource(header, "temp", 0, fileChannel, dataAddress);
 * HdfFixedPoint[] rawData = rawSource.readAll();
 * rawSource.stream().forEach(d -> System.out.println(d.toBigInteger()));
 * </pre>
 */
public class FixedPointDataSource extends AbstractFixedPointDataSource<HdfFixedPoint> {
    /**
     * Constructs a FixedPointDataSource for reading raw data from an HDF5 file.
     *
     ...
     */
    public FixedPointDataSource(HdfObjectHeaderPrefixV1 headerPrefixV1, int scale, FileChannel fileChannel, long startOffset) {
     ...
     */
    public HdfFixedPoint[] readAll() throws IOException {
        if (fileChannel == null) {
