 ...
 * </p>
 */
public class HdfDimensionsRead {
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(HdfDimensionsRead.class);
    /**
    ...
    private void run() {
        try {
            ...
            filePath = getResourcePath("array.h5");
            try (SeekableByteChannel channel = Files.newByteChannel(filePath, StandardOpenOption.READ)) {
                HdfFileReader reader = new HdfFileReader(channel).readFile();
                log.debug("Root Group: {} ", reader.getRootGroup());
                try (HdfDataSet ds = reader.getRootGroup().getDataset("/array_dataset").orElseThrow()) {
                    displayScalarData(channel, ds, HdfArray.class, reader);
                }
            }
        } catch (Exception e) {
