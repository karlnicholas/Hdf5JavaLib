...
//        HdfGroupSymbolTableNode snod = snodEntry;
    ...
    public Optional<HdfDataObject> findObjectByName(String name, HdfGroup group) {
        ...
        if (nodeLevel == 0) {
            ...
            Optional<HdfBTree> childBtreeEntry = entries.get(entryIndex).getChildBTree();
            ...
            HdfBTree childBTree = childBtreeEntry.get();
...
//        }
     ...
     */
    private void collectSnodsRecursively(HdfBTree node, Map<Long, HdfGroupSymbolTableNode> map) {
        ...
        if (node.nodeLevel == 0) {
            ...
            for (HdfBTreeEntry entry : entries) {
                Optional<HdfBTree> childBtree = entry.getChildBTree();
